...
class Voyager(tf.keras.Model):
    ...
    def compile_model(config, num_unique_pcs, num_unique_pages):
        ...
        '''
        model = Voyager(config, num_unique_pcs, num_unique_pages)
...
class OffsetVoyager(tf.keras.Model):

    def __init__(self, config, pc_vocab_size, page_vocab_size):
        super(OffsetVoyager, self).__init__()
        # Needed for stateless dropout seed
        self.steps_per_epoch = config.steps_per_epoch
        self.step = 0
        self.epoch = 1
    ...
    def train_step(self, data):
        ret = super(OffsetVoyager, self).train_step(data)
    ...
    def compile_model(config, num_unique_pcs, num_unique_pages):
        ...
        Create and compile the model
        '''
        model = OffsetVoyager(config, num_unique_pcs, num_unique_pages)
        num_offsets = (1 << config.offset_bits)
...
class OffsetLSTM(tf.keras.Model):

    def __init__(self, config, pc_vocab_size, page_vocab_size):
        super(OffsetLSTM, self).__init__()
        # Needed for stateless dropout seed
        self.steps_per_epoch = config.steps_per_epoch
        self.step = 0
        self.epoch = 1
    ...
    def train_step(self, data):
        ret = super(OffsetLSTM, self).train_step(data)
    ...
    def compile_model(config, num_unique_pcs, num_unique_pages):
        ...
        Create and compile the model
        '''
        model = OffsetLSTM(config, num_unique_pcs, num_unique_pages)
        num_offsets = (1 << config.offset_bits)
